...
# melt rate, bottom water temperature, bottom water salinity, surface velocity)
# where each variable is averaged over the years 2002-2016 and zoomed into
# the region of interest. The 3 parts of each figure are MetROMS, FESOM
# low-res, and FESOM high-res. The surface velocity figures show absolute value
...
def mip_regions_1var ():
    ...
    var_names = ['draft', 'melt', 'temp', 'salt', 'vel']
    # Constants
    ...
    for var in var_names:
        ...
        if var == 'draft':
            ...
            if var == 'melt':
                ...
                num_lat_u = size(u_tmp,0)
                num_lon_u = size(u_tmp,1)
                for j in range(1,num_lat_u-1):
                    for i in range(1,num_lon_u-1):
                        # Check for masked points
                        if u_tmp[j,i] is ma.masked:
                            # Look at 4 neighbours
                            neighbours = ma.array([u_tmp[j-1,i], u_tmp[j,i-1], u_tmp[j+1,i], u_tmp[j,i+1]])
                            # Find how many of them are unmasked
                            num_unmasked = MaskedArray.count(neighbours)
                            if num_unmasked > 0:
                                # There is at least one unmasked neighbour;
                                # set u_tmp to their average
                                u_tmp[j,i] = sum(neighbours)/num_unmasked
                # Repeat for v
                num_lat_v = size(v_tmp,0)
                num_lon_v = size(v_tmp,1)
                for j in range(1,num_lat_v-1):
                    for i in range(1,num_lon_v-1):
                        if v_tmp[j,i] is ma.masked:
                            neighbours = ma.array([v_tmp[j-1,i], v_tmp[j,i-1], v_tmp[j+1,i], v_tmp[j,i+1]])
                            num_unmasked = MaskedArray.count(neighbours)
                            if num_unmasked > 0:
                                v_tmp[j,i] = sum(neighbours)/num_unmasked
                # Interpolate to rho grid and rotate
                u_rho, v_rho = rotate_vector_roms(u_tmp, v_tmp, roms_angle)
                # Get speed
        ...
        if var != 'draft':
            if var == 'melt':
                ...
                # Read rotated lat and lon for each node
                ...
                node_ur_lr = id.variables['u'][0,0:fesom_n2d_lr]
                node_vr_lr = id.variables['v'][0,0:fesom_n2d_lr]
                # Unrotate
        ...
        if var != 'draft':
            if var == 'melt':
                ...
                node_ur_hr = id.variables['u'][0,0:fesom_n2d_hr]
                node_vr_hr = id.variables['v'][0,0:fesom_n2d_hr]
